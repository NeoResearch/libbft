...
simpleExample()
{
   auto initial = std::shared_ptr<State<Data>>(new State<Data>(false, "Initial"));
   ...
   initial->addTransition(std::shared_ptr<Transition<Data>>((new Transition<Data>(final, "after1sec"))->add(Condition<Data>("C >= 3",
      [](const Timer& c, TData d, MachineId) -> bool {
...
simpleMultiMachineExample(int id)
{
   ...
   auto started = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "Started"));
   auto backup = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "Backup"));
   auto primary = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "Primary"));
   auto reqSentOrRecv = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "RequestSentOrReceived"));
   auto commitSent = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "CommitSent"));
//   auto viewChanging = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "ViewChanging"));
   auto blockSent = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(true, "BlockSent"));
   ...
   // initial -> backup
   started->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(backup))->add(
   ...
   // initial -> primary
   started->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(primary))->add(Condition<McDBFT>("(H+v) mod R = i",
      [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
   ...
   backup->addTransition(std::shared_ptr<Transition<McDBFT>>(toReqSentOrRecv1->add(Condition<McDBFT>("OnPrepareRequest",
      [](const Timer &t, TMcDBFT d, MachineId me) -> bool {
   ...
   reqSentOrRecv->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(commitSent))->add(Condition<McDBFT>("(H+v) mod R = i",
      [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
   ...
   commitSent->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(blockSent))->add(Condition<McDBFT>("(H+v) mod R = i",
      [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
...
dbft_test_real_dbft2_primary()
{
   ...
   auto machine = std::shared_ptr<dBFT2Machine>(new dBFT2Machine(0, 1)); // f=0, N=1
...
dbft_test_backup_multi()
{
   ...
   auto preinitial = std::shared_ptr<State<McDBFT>>(new State<McDBFT>(false, "PreInitial"));
   ...
   */
   ...
   // preinitial -> started
   preinitial->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(machine0->getStateByName("Started")))->add(Condition<McDBFT>(
   ...
   auto minitial = std::shared_ptr<MultiState<dBFT2Context>>(new MultiState<dBFT2Context>(1, nullptr));
...
commit_phase_dbft2(int id_me)
{
   ...
   // preinitial -> started
   preinitial->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(started))->add(Condition<McDBFT>("OnStart()",
      [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
         //cout << "Waiting for OnStart..." << endl;
   ...
   // initial -> backup
   started->addTransition(std::shared_ptr<Transition<McDBFT>>((new Transition<McDBFT>(backup))->add(Condition<McDBFT>("not (H+v) mod R = i",
      [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
         //cout << "lambda1" << endl;
   ...
   // backup -> reqSentOrRecv
   auto toReqSentOrRecv1 = new Transition<McDBFT>(reqSentOrRecv);
   backup->addTransition(std::shared_ptr<Transition<McDBFT>>(toReqSentOrRecv1->add(Condition<McDBFT>("OnPrepareRequest(v,H)",
      [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
   ...
   ))->add(Condition<McDBFT>("ValidBlock", [](const Timer& t, TMcDBFT d, MachineId me) -> bool {
      // always good block right now
      return true;
   }))->add(Action<McDBFT>("send: PrepareResponse(v, H)", [](Timer& C, TMcDBFT d, MachineId me) -> void {
   ...
   primary->addTransition(std::shared_ptr<Transition<McDBFT>>(primToReqSentOrRecv1->add(Condition<McDBFT>("C >= T?",
      [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
         // C >= T?
         return C.elapsedTime() >= d->getParams(me.id)->T;
      }
   ))->add(Action<McDBFT>("send: PrepareRequest(v, H)", [](Timer& C, TMcDBFT d, MachineId me) -> void {
      cout << "sending PrepareRequest from " << me.id << " for view " << d->getParams(me.id)->v << endl;
      // TODO: attach  block hash as well?
      vector<string> evArgs = { std::to_string(d->getParams(me.id)->v), std::to_string(d->getParams(me.id)->H) };
      d->broadcast("PrepareRequest", me, evArgs);
   }))->add(Action<McDBFT>("C := 0", [](Timer& C, TMcDBFT d, MachineId me) -> void {
      C.reset();
   }))));

   // primary -> viewChanging
   auto primToViewChanging = new Transition<McDBFT>(viewChanging);
   primary->addTransition(std::shared_ptr<Transition<McDBFT>>(primToViewChanging->add(Condition<McDBFT>("(C >= T exp(v+1))?",
      [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
         // C >= T?
   ...
   ))->add(Action<McDBFT>("send: ChangeView(v+1, H)", [](Timer& C, TMcDBFT d, MachineId me) -> void {
   ...
   backup->addTransition(std::shared_ptr<Transition<McDBFT>>(backpToViewChanging->add(Condition<McDBFT>("(C >= T exp(v+1))?",
      [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
      // C >= T?
      //cout << "evaluate primary view change... C = " << C.elapsedTime() << " ~ T_limit = " << (d->params->T * std::pow(2, d->params->v+1)) << std::endl;
      return C.elapsedTime() >= (d->getParams(me.id)->T * std::pow(2, d->getParams(me.id)->v + 1));
   ...
   reqSentOrRecv->addTransition(std::shared_ptr<Transition<McDBFT>>(reqsrToViewChanging->add(Condition<McDBFT>("(C >= T exp(v+1))?",
         [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
         // C >= T?
         //cout << "evaluate primary view change... C = " << C.elapsedTime() << " ~ T_limit = " << (d->params->T * std::pow(2, d->params->v+1)) << std::endl;
         return C.elapsedTime() >= (d->getParams(me.id)->T * std::pow(2, d->getParams(me.id)->v + 1));
   ...
   reqSentOrRecv->addTransition(std::shared_ptr<Transition<McDBFT>>(reqsrToCommitSent->add(Condition<McDBFT>("EnoughPreparations",
      [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
   ...
   ))->add(Action<McDBFT>("send: Commit(v, H)", [](Timer& C, TMcDBFT d, MachineId me) -> void {
   ...
   commitSent->addTransition(std::shared_ptr<Transition<McDBFT>>(commitSentToBlockSent->add(Condition<McDBFT>("EnoughCommits",
      [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
         vector<std::shared_ptr<Event>> events = d->getEvents(me.id);
   ...
   ))->add(Action<McDBFT>("send: BlockRelay(v, H)", [](Timer& C, TMcDBFT d, MachineId me) -> void {
   ...
   viewChanging->addTransition(std::shared_ptr<Transition<McDBFT>>(viewChToStarted->add(Condition<McDBFT>("EnoughViewChanges",
      [](const Timer& C, TMcDBFT d, MachineId me) -> bool {
         vector<std::shared_ptr<Event>> events = d->getEvents(me.id);
   ...
   ))->add(Action<McDBFT>("v := v + 1", [](Timer& C, TMcDBFT d, MachineId me) -> void {
