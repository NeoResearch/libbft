...
#ifndef INCLUDE_LIBBFT_DBFT2_DBFT2MACHINE_HPP_
#define INCLUDE_LIBBFT_DBFT2_DBFT2MACHINE_HPP_
...
class dBFT2Machine : public ReplicatedSTSM<dBFT2Context> {
 public:
  using TSingleTimerStateMachine =
      std::shared_ptr<SingleTimerStateMachine<MultiContext<dBFT2Context>>>;
   ...
   * N=7; ...) */
  explicit dBFT2Machine(int _f = 0, int N = 1, TClock _clock = nullptr,
                        MachineId _me = MachineId(),
                        std::string _name = "replicated_dBFT")
      : ReplicatedSTSM<dBFT2Context>(std::move(_clock), std::move(_me),
                                     std::move(_name)),
        f(_f) {
    assert(f >= 0);  // NOLINT
    assert(N >= 1);  // NOLINT
          ...
          std::shared_ptr<SingleTimerStateMachine<MultiContext<dBFT2Context>>>(
              new SingleTimerStateMachine<MultiContext<dBFT2Context>>(
                  std::unique_ptr<Timer>(
                      new Timer("C", clonePtr(*this->clock))),
                  MachineId(i), clonePtr(*this->clock), "dBFT"));
    }
  ...
  dBFT2Machine(int _f, std::vector<TSingleTimerStateMachine> _machines,
               TClock _clock = nullptr, int _me = 0,
               std::string _name = "replicated_dBFT")
      : ReplicatedSTSM<dBFT2Context>(std::move(_clock), MachineId(_me), _name),
        f(_f) {
    this->machines = std::move(_machines);
    assert(f >= 0);                                        // NOLINT
    assert(!this->machines.empty());                       // NOLINT
    assert(f <= static_cast<int>(this->machines.size()));  // NOLINT
  }
  ...
  void fillSimpleCycle(int m) {
    ...
    preinitial->addTransition(
        std::shared_ptr<Transition<MultiContext<dBFT2Context>>>(
            (new Transition<MultiContext<dBFT2Context>>(started))
                ->add(Condition<MultiContext<dBFT2Context>>(
                    "OnStart()",
                    [](const Timer& t, TMultiContext d, MachineId _me) -> bool {
                      std::cout << "Waiting for OnStart..." << std::endl;
                      return d->hasEvent("OnStart", _me.id,
    ...
    started->addTransition(
        std::shared_ptr<Transition<MultiContext<dBFT2Context>>>(
            (new Transition<MultiContext<dBFT2Context>>(backup))
                ->add(Condition<MultiContext<dBFT2Context>>(
                    "not (H+v) mod R = i",
                    [](const Timer& t, TMultiContext d, MachineId _me) -> bool {
                      std::cout << "lambda1" << std::endl;
                      return (d->vm[_me.id].params->H +
                              d->vm[_me.id].params->v) %
                                 d->vm[_me.id].params->R !=
                             _me.id;
                    }))));
    ...
    started->addTransition(
        std::shared_ptr<Transition<MultiContext<dBFT2Context>>>(
            (new Transition<MultiContext<dBFT2Context>>(primary))
                ->add(Condition<MultiContext<dBFT2Context>>(
                    "(H+v) mod R = i",
                    [](const Timer& t, TMultiContext d, MachineId _me) -> bool {
                      std::cout << "lambda2 H=" << d->vm[_me.id].params->H
                                << " v=" << d->vm[_me.id].params->v
                                << " _me=" << _me.id << std::endl;
                      return (d->vm[_me.id].params->H +
                              d->vm[_me.id].params->v) %
                                 d->vm[_me.id].params->R ==
    ...
    auto toReqSentOrRecv1 =
        new Transition<MultiContext<dBFT2Context>>(reqSentOrRecv);
    backup->addTransition(
        std::shared_ptr<Transition<MultiContext<dBFT2Context>>>(
            toReqSentOrRecv1->add(Condition<MultiContext<dBFT2Context>>(
                "OnPrepareRequest(v)",
                [](const Timer& t, TMultiContext d, MachineId _me) -> bool {
                  std::cout << "waiting for event OnPrepareRequest at "
                            << _me.id << " for view " << d->vm[_me.id].params->v
                            << std::endl;
                  std::stringstream ss;
    ...
    reqSentOrRecv->addTransition(
        std::shared_ptr<Transition<MultiContext<dBFT2Context>>>(
            (new Transition<MultiContext<dBFT2Context>>(commitSent))
                ->add(Condition<MultiContext<dBFT2Context>>(
                    "(H+v) mod R = i",
                    [](const Timer& t, TMultiContext d, MachineId _me) -> bool {
                      std::cout << "nothing to do... assuming all preparations "
                                   "were received!"
                                << std::endl;
                      return true;
                    }))));
    ...
    commitSent->addTransition(std::shared_ptr<
                              Transition<MultiContext<dBFT2Context>>>(
        (new Transition<MultiContext<dBFT2Context>>(blockSent))
            ->add(Condition<MultiContext<dBFT2Context>>(
                "(H+v) mod R = i",
                [](const Timer& t, TMultiContext d, MachineId _me) -> bool {
                  std::cout
                      << "nothing to do... assuming all commits were received!"
                      << std::endl;
                  return true;
                }))));
  }
  ...
  TMultiState initialize(TMultiState current, TMultiContext p) override {
    if (!current) {
      current = std::shared_ptr<MultiState<dBFT2Context>>(
          new MultiState<dBFT2Context>(machines.size(), nullptr));
    }
...
#endif  // INCLUDE_LIBBFT_DBFT2_DBFT2MACHINE_HPP_
